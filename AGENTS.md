...
| `usage` | Optional | Reconciles (may write), then calls the **undocumented** `GET /api/oauth/usage` per slot for 5h / 7d percentages. `-Json` for scripted output, `-Watch` (`-Interval <seconds>`, floor 60) for a live view. With `<name>`, renders a verbose single-slot block. |
