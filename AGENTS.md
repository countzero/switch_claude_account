...
- **Supported platforms**: Windows, Linux, and macOS, all three covered by the CI matrix. macOS was refused until 4.1.0 on the belief that Claude Code kept credentials in the Keychain there; it does not, and the block comment under `# --- Where Claude Code actually keeps the active login ---` records what it does instead and the one flag that could change it.
...
| `install` / `uninstall` | No | Adds / removes the wrapper function and aliases in the PowerShell profile. `uninstall` is the one action exempt from `Assert-CredentialDir`: it touches nothing else, so it must stay able to undo itself on a machine the other actions refuse. |
