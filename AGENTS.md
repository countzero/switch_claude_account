...
- **`CLAUDE_CONFIG_DIR`**: taken verbatim, no `~` expansion and no relative-path resolution, because Claude Code does neither (anthropics/claude-code#78988). Normalizing would aim `sca` at a different directory than the `claude` process it mirrors. `Get-ConfigDirAdvisory` prints one line when the relocation strands slots in the default directory, and stays silent otherwise, because the variable is a permanent setting and an unconditional line would train the user to ignore it.
