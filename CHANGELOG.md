...
- A single slow `/api/oauth/usage` response no longer erases a slot's numbers. A failed read falls back to the last known percentages instead of collapsing the row to `error: The request was canceled due to the configured HttpClient.Timeout...`. The endpoint answers in 46-2108 ms in practice, so the previous shared 5-second budget left almost no headroom; usage now gets 12 seconds and the token refresh gets its own 15. Since slots are polled serially and the loop cannot repaint mid-poll, one degraded slot can freeze a watch frame for 12 seconds on a usage timeout, 15 on a refresh timeout (the usage call is never made in that case), or up to 27 if a slow-but-successful refresh is followed by a usage timeout. That cost is also why a timeout is never retried. The same fallback covers a token refresh that dies in transport, which carries the largest budget of the three calls and is therefore the likeliest place for a blip to land. The cache is reached only by a failure that says nothing about the request: no HTTP status from the HTTP stack itself, or a `5xx`. A `4xx` from either endpoint is reported as the failure it is, because `invalid_grant` means the refresh token is rejected for good and a `4xx` from the usage endpoint is how an endpoint change after a Claude Code upgrade would first show itself; serving either from cache would paint fresh-looking numbers under an `ok` status for the life of the cache entry. A refresh response missing `access_token` or `expires_in`, and a slot-file write that fails after the server already rotated the refresh token, are hard failures for the same reason: they carry no HTTP status either, but the slot behind them is not fine.
- When nothing is cached, a failed usage read retries once only if a second immediate attempt can plausibly answer differently: a `5xx` (Anthropic's `529 Overloaded` clears in seconds) or a codeless transport failure. A timeout is not retried, because it has already spent the full budget and slots are polled serially, so retrying it doubled every slot's contribution to the first frame of a watch; nor is a `4xx`, which the server rejects identically the second time. Whatever the retry itself returns is classified on its own merits rather than inheriting the label of the failure that scheduled it. That matters most on the rate-limit path, which is reached whenever a slot has no cached reading at all, so every slot on a watch's first poll: a `4xx` from an endpoint that drifted after a Claude Code upgrade used to come back reported as a rate limit, with no status and no message, wearing the label of the one failure that clears on its own.
- Auto-rotation reads a slot's cached percentages when a live read fails, instead of treating every non-`ok` row as 0% utilized. A throttled or briefly unreachable active slot at 100% now rotates rather than freezing. Rotation still refuses to move *into* a slot it could not verify, though a destination may be entered on a reading up to the cache TTL old; requiring a strictly live read there would let one blip disqualify every healthy peer and freeze rotation exactly when it is wanted.
- A cached reading is no longer served without an upper age bound. Falling back to last-known percentages was argued for a brief outage and then applied at any age, which was not merely cosmetic: `sca monitor` reports that the active slot's usage is unknown only when the row carries no data at all, so an unbounded entry left a permanently unreadable active slot being judged on a days-old reading while the footer said rotation was armed. Past six hours, one session window plus slack, the reading is dropped and the failure is reported instead.
- A usage bucket whose reset time has already passed is dropped from the reading as that reading is built, rather than being discounted by each view separately. A bucket carries the utilization of a window that *ends* at its reset time, so once that instant passes the number describes a window the account is no longer in; the endpoint never returns one, but the per-process cache can. Removing it at the single point every reading is constructed is what makes the table cell, the plan status, the aggregate bars, the terminal title and the rotation decision agree: "rolled" arrives everywhere as "absent", which each of them already renders as an em-dash or counts as 0%. Discounting it per consumer instead had one row print `100% now` in the Session cell and a red `limited 5h` in Status while the bar directly above it read 0%, the title read `[!] 100%`, and auto-rotation treated the same slot as idle and refused to move off it.
...
- The usage advisory prints one line per condition instead of letting the cache-fallback line suppress everything else, and distinguishes a failed live read from a limit. Its condition lines name no source, because the same renderer also draws warmup frames whose rows come from `claude -p` rather than the usage API; the per-slot reason line below carries the actual cause. The block is bounded at eight lines, because it is painted inside a watch frame redrawn by cursor-home plus per-line erase, which stops overwriting in place the moment the frame outgrows the terminal. The bound is spent worst-value-first: condition lines name every affected slot and the per-status remedies are the only actionable text, so those two groups are reserved and always fit, and the per-slot detail lines take what is left. Reserving the remedies *before* choosing the detail lines is load-bearing rather than tidy, since suppressing a slot's detail is what makes its remedy print at all.
- `sca usage -Json` may now emit `data` on a row whose `status` is `"error"`, and `error` on a row whose `status` is `"ok"`. Both follow from the same rule: `is_cached_fallback: true` is the only freshness marker, and a row served from a recent cache keeps its live-quality status while still reporting what the failed live read said. A failure reason is reported in full; the bound belongs to each renderer, which applies it at its own width. A `data` block may also omit `five_hour` or `seven_day` entirely when that window has rolled since the reading was taken: the key is absent rather than present-and-null, so a script testing for it sees what every on-screen view sees.

### Added

- A test workflow running the suite on `windows-latest`, `ubuntu-latest` and `macos-latest`. No workflow ran the tests before. Coverage and its 90% gate run on Windows only, because the gate counts one run and each platform necessarily leaves the others' branches untouched. A concurrency group keyed on the ref keeps a push to a branch with an open pull request from running the whole matrix twice for one commit.
...
- Credential files left readable by other users are tightened to `0600` on the first run of any action, and the count is reported. Writing new files correctly does not heal the ones already on disk: a slot that is never re-saved and whose token never refreshes would keep a world-readable refresh token indefinitely, and `sca switch` rewrites only `.credentials.json`. The pass covers the files `sca` creates under the credentials directory. `~/.claude.json` is excluded, because it is Claude Code's file and is rewritten by it through its own atomic rename, so repairing it would re-tighten and re-announce after every session rather than heal an installed base once; `sca` still writes its own bytes there at `0600`. Symlinks are skipped, since `chmod` follows them and would re-permission a target outside the directory being repaired. The line reports what was changed rather than blaming an older version, which `sca` has no way to establish: a restore, a sync tool or a hand-run `chmod` leaves the same finding.
- A credentials directory that `sca` creates is now `0700`, because slot filenames carry the account's email address. One that already exists is left exactly as it is, mode included, since it is usually Claude Code's own and re-permissioning another tool's directory is not this tool's call. On any machine where Claude Code ran first, which is most of them, that means the exposure is **not** closed and `chmod 700 ~/.claude` is the user's to run; the README says so.
...
- `sca monitor` no longer goes silently inert when the active slot's usage cannot be read. It previously kept displaying the last `Rotated from ... to ... at ...` line while being structurally unable to rotate; it now reports `[Monitor] Active slot usage unknown (<status>); rotation paused.`, and returns to `[Monitor] Automatic slot switching is enabled.` on the first poll that can judge the slot again, so the paused line never outlives the state it describes. Only a poll that actually judged the active row clears it: a snapshot with no slots, or none marked active, cannot rotate either, and announcing it as armed would swap one false statement for the other.
