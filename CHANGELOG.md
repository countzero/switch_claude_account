...
- A single slow `/api/oauth/usage` response no longer erases a slot's numbers. A failed read falls back to the last known percentages instead of collapsing the row to `error: The request was canceled due to the configured HttpClient.Timeout...`. The endpoint answers in 46-2108 ms in practice, so the previous shared 5-second budget left almost no headroom; usage now gets 12 seconds and the token refresh gets its own 15. The same fallback covers a token refresh that dies in transport, which carries the largest budget of the three calls and is therefore the likeliest place for a blip to land. A `4xx` from the refresh is still reported as `expired`, because `invalid_grant` means the token is rejected for good and a cached reading would hide a slot that needs re-authentication.
...
- The aggregate bars now count a slot whose percentages came from the cache fallback. The table already printed those numbers and rotation already acted on them, so excluding the same row from the pool mean made the bars contradict the rows directly beneath them, and a pool whose reads had all gone stale lost both bars entirely while every row still showed a percentage.
